...
public class ExcelToHtml {
    ...
    public ExcelToHtml(final InputStream in, int sheetIndex) throws IOException {
        sdf = new SimpleDateFormat("dd/MM/yyyy");
        if (in == null) {
            book = null;
            palette = null;
        ...
        evaluator = book.getCreationHelper().createFormulaEvaluator();
            ...
            customTable(book.getSheetAt(sheetIndex));
    ...
    private void customTable(final HSSFSheet sheet) {
        if (sheet == null) {
            return;
        }
        if (sheet.getDrawingPatriarch() != null) {
        ...
        for (rowIndex = 87; rowIndex < 115; ++rowIndex) {
            tr(sheet.getRow(rowIndex));
        }
        out.append("</table>\n");
    }
    ...
    private void td(final HSSFCell cell) {
        ...
        final short[] backRGB = palette.getColor(style.getFillForegroundColor()).getTriplet();
